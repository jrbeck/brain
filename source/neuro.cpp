...
void grow_synapses (void);
void remove_synapses (void);
int shutdown (void);
void flush_neurons (void);
void main_loop (void);
...
int main (int argc, char *argv[]) {
	...
	if (initialize ()) return 1; // bail if init fails
...
// initialize the colors for display elements
void init_colors (void) {
...
//	init_inputs ();
	...
	// must be done after all the neurons have been placed
	//grow_synapses ();
...
void grow_neurons (v2d_t tl, v2d_t br, int num) {
	...
	for (int i = 0; i < num; i++) {
		...
		// now push the neuron onto the global list
		neuron.push_back (n);
	}
}
...
void handle_mouse_button (int button, v2d_t pos, int mode) {
	if (mode == MODE_VIEW) {
		if (button == SDL_BUTTON_WHEELUP) {
			...
			view_bl = v2d_sub (wp, v2d_scale (v2d_sub (wp, view_bl), .8333));
			view_tr = v2d_add (wp, v2d_scale (v2d_sub (view_tr, wp), .8333));
